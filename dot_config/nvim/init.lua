...
local function apply_theme()
	...
	if status and type(omarchy) == "table" and omarchy.colorscheme then
		-- Map colorscheme name → opt plugin directory name so packadd can put
		-- the plugin's lua/ on package.path before the colorscheme file loads.
		local plugin_map = {
			...
			["rose-pine-dawn"]   = "neovim",
			["ashen"]            = "ashen.nvim",
		}

		local plugin = plugin_map[omarchy.colorscheme]
		if plugin then
			pcall(vim.cmd, "packadd " .. plugin)
		end

		local success, _ = pcall(vim.cmd, "colorscheme " .. omarchy.colorscheme)
		if not success then
			vim.cmd("colorscheme miniwinter") -- Fallback if colorscheme command fails
		end
	else
		-- Fallback if the file is blank, missing, or returns a boolean true
